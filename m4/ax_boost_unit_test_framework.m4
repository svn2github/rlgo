...
# DESCRIPTION
#
#   Test for Unit_Test_Framework library from the Boost C++ libraries. The macro
...
[
	...
	AS_HELP_STRING([--with-boost-unit-test-framework@<:@=special-lib@:>@],
                   [use the Unit_Test_Framework library from boost - it is possible to specify a certain library for the linker
                        e.g. --with-boost-unit-test-framework=boost_unit_test_framework-gcc-mt ]),
        [
	...
	if test "x$want_boost" = "xyes"; then
		...
		if test "x$ax_cv_boost_unit_test_framework" = "xyes"; then
            ...
            if test "x$ax_boost_user_unit_test_framework_lib" = "x"; then
                for libextension in `ls $BOOSTLIBDIR/libboost_unit_test_framework*.{so,a}* 2>/dev/null | sed 's,.*/,,' | sed -e 's;^lib\(boost_unit_test_framework.*\)\.so.*$;\1;' -e 's;^lib\(boost_unit_test_framework.*\)\.a*$;\1;'` ; do
                     ax_lib=${libextension}
