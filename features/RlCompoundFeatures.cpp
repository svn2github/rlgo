...
void RlCompoundTracker::Execute(SgMove move, SgBlackWhite colour, bool execute)
{
    RlTracker::Execute(move, colour, execute);
    for (int i = 0; i < ssize(m_trackers); ++i)
        if (m_trackers[i]->Tock() != Tock())
            m_trackers[i]->Execute(move, colour, execute);
}
...
bool RlCompoundTracker::SupportUndo() const
{
    for (int i = 0; i < ssize(m_trackers); ++i)
        if (m_trackers[i]->Tock() != Tock())
            if (!m_trackers[i]->SupportUndo())
                return false;
    return true;
}
...
void RlCompoundTracker::TrackEval(int q, RlFloat eval, 
    bool execute, bool incremental)
{
    RlTracker::TrackEval(q, eval, execute, incremental);
    for (int i = 0; i < ssize(m_trackers); ++i)
        if (m_trackers[i]->Tock() != Tock())
            m_trackers[i]->TrackEval(q, eval, execute, incremental);
}
